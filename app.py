...
def score_color(score):
    ...
    elif score >= 45: return "color-yellow"
...
<div class="hero">
    ...
    <p style="font-size:0.78rem; color:#5A6080; margin-top:0.8rem;">
        ...
        <a href="https://rabikharel.me" style="color:#00D4AA;">Portfolio</a>
    </p>
</div>
""", unsafe_allow_html=True)
    ...
    if not uploaded:
        ...
        with c3:
            ...
            st.markdown(f"""<div class="metric-card">
                ...
                <div class="metric-sub color-purple">{keyword_score}% match</div>
            </div>""", unsafe_allow_html=True)
        ...
        if overall >= 70:
            st.markdown('<div class="tip-card">🎉 <strong>Excellent match!</strong> Your resume aligns very well with this job. Apply with confidence!</div>', unsafe_allow_html=True)
        elif overall >= 45:
            st.markdown('<div class="tip-card">📝 <strong>Good foundation!</strong> Adding the missing keywords above could boost your score by 20-30 points.</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="tip-card">🔧 <strong>Needs tailoring.</strong> Customize your resume for this specific role by adding missing keywords.</div>', unsafe_allow_html=True)

        if ats_score < 75:
            missing_ats = [label for label, passed in ats_checks if not passed]
            st.markdown(f'<div class="tip-card">🤖 <strong>ATS Warning:</strong> Fix these sections: <strong>{", ".join(missing_ats)}</strong></div>', unsafe_allow_html=True)

        if missing_kw:
            top5 = ", ".join(f"<strong>{k}</strong>" for k in missing_kw[:5])
            st.markdown(f'<div class="tip-card">🔑 <strong>Priority Keywords:</strong> Add these immediately: {top5}</div>', unsafe_allow_html=True)

        if not re.search(r'\d+%|\d+ years|\d+\+', resume_raw):
            st.markdown('<div class="tip-card">📊 <strong>Add Numbers:</strong> Quantify achievements! Example: "Improved accuracy by 15%" or "Built chatbot serving 200+ users"</div>', unsafe_allow_html=True)

        if not re.search(r'github|linkedin|portfolio', resume_raw.lower()):
            st.markdown('<div class="tip-card">🔗 <strong>Add Links:</strong> Include your GitHub and Portfolio (rabikharel.me) — AI/ML recruiters always check these!</div>', unsafe_allow_html=True)

        st.markdown('<div class="tip-card">🎯 <strong>Pro Tip:</strong> Tailor your resume for EACH job application using the exact phrases from the job description.</div>', unsafe_allow_html=True)
